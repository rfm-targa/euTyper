...
def check_arguments(args):
    """ Checks the given parameters so far if it is in the right format.
        ...
        ----------
        inputs : list
            A list with the following agurments in order
            - input files path
            - output file path
    ...
    # check input data #
    ####################
    # check weither the sample_metadata is in the right format and corrosponds with the deliverd input data
    # check if sample metadata contains files,

    #### TO DO ###
    # have to figure out how to check for augustus species request with possibility to custome training files
    ...
    if os.path.isfile(args[1]):
        print(' output directory path is linked to a path, please change : ' + (args[1]))
        exit()
...
def seed_creation(output_directory, sample_metadata_path, MinSeqlen, word_size, window_size, clustering_sim,
    ...
    # create scheme seed
    files = []
    # retrieve file names which should be already run by Augustus
...
def main(args):
    ...
    ##############################################
    results = seed_creation(output_path, sample_metadata_path, min_seq_length, word_size, window_size, clustering_sim,
